...
class Thim_Ekit_Widget_Loop_Tour_Sale extends Widget_Base {
	use Widget_Loop_Trait;

	public function get_name() {
		return 'thim-loop-tour-sale';
	}

	public function get_title() {
		return esc_html__( 'Tours Sale Flash', 'travel-booking' );
	}
	...
	public function show_in_panel() {
		...
		if ( ( ! empty( $post_type ) && $post_type == 'tours' ) || $type == 'single-tour' ) {
			return true;
		}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'display',
			array(
				'label'   => esc_html__( 'Display', 'travel-booking' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'text',
				'options' => array(
					'text'       => esc_html__( 'Text', 'travel-booking' ),
					'percentage' => esc_html__( 'Percentage', 'travel-booking' ),
				),
			)
		);
		$this->add_control(
	...
	protected function render() {
		...
		if ( $product->is_on_sale() ) :
			...
			if ( $percentage ) {
				echo '<span class="ekit-onsale">' . sprintf(
					_x( '-%d%%', 'sale percentage', 'travel-booking' ),
					$percentage
				) . '</span>';
			} else {
